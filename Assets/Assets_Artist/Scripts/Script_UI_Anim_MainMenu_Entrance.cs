...
public class Script_UI_Anim_MainMenu_Entrance : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        header.transform.localPosition = new Vector3(-1250, 0, 0); // This sets it to off screen north
    ...
    {
        float newPosition = header.transform.localPosition.x;
        float startPosition = header.transform.localPosition.x;
        float elapsedTime = 0;

        while (elapsedTime < dlMovement)
        {
            newPosition = Mathf.Lerp(startPosition, 0, elapsedTime / dlMovement);
            header.transform.localPosition = new Vector3(newPosition, 0, 0);
            elapsedTime += Time.deltaTime;
            yield return null;
        }
    }
    ...
    IEnumerator HeaderAnimateOff() // Moves the header off screen
    {
        ...
        while (elapsedTime < dlMovement)
        {
            newPosition = Mathf.Lerp(startPosition, -1250, elapsedTime / dlMovement);
            header.transform.localPosition = new Vector3(newPosition, 0, 0);
            elapsedTime += Time.deltaTime;
            yield return null;
        }
